...
def curves_loss_and_accuracy(loss, val_loss, plotsdir, case):
...
def report(preds, truth, plotsdir, case):
    pandas.DataFrame(classification_report(truth, preds, output_dict=True))[["macro avg", "accuracy"]].to_csv(plotsdir / f"{case}_report.csv", index=False)
