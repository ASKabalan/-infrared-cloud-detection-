...
def main(args):
    ...
    ds_train_gen , ds_val_gen = load_dataset(args.input_folder_path,args.train_batch_size,args.aug_batch_size,args.val_batch_size)
    # Train the model
    ...
    state = create_train_state(init_rng, input_shape, args.unet_conv, args.bottle_neck_conv, args.learning_rate, n_batches)
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.verbose >= 1:
            print("\r", end="")
            print(f"Epoch {epoch + 1}/{args.num_epochs} done - Loss: {metrics.avg_loss}, Accuracy: {metrics.avg_accuracy}, - val_Loss: {metrics.avg_val_loss}, val_Accuracy: {metrics.avg_val_acc},  Time taken: {epoch_time:.2f} seconds")

        if metrics.avg_val_loss < best_val_loss:
            best_val_loss = metrics.avg_val_loss
            counter = 0
            # Save the model checkpoint
            save_model(state, args.output_model_path,epoch)
        else:
            counter += 1
            if args.verbose >= 1:
